...
function paginateChecklist(invoice: Invoice): VirtualChecklistPage[] {
  ...
  fullCategories.forEach(cat => {
    totalRows += 1 + cat.items.length
  })

  // Single page threshold: 12 or fewer rows fit cleanly on 1 page with signature block
  if (totalRows <= 12) {
    return [{
  ...
  // Multi-page threshold: fill Page 1 with target ~12-14 rows
  const p1TargetRows = Math.min(14, Math.max(10, Math.ceil(totalRows / 2)))

  const page1Categories: CategorySlice[] = []
  const page2Categories: CategorySlice[] = []

  let p1RowsCount = 0

  for (const cat of fullCategories) {
    const catRows = 1 + cat.items.length
    if (p1RowsCount + catRows <= p1TargetRows + 2 || p1RowsCount === 0) {
      page1Categories.push({ ...cat, isContinued: false })
      p1RowsCount += catRows
    } else if (p1RowsCount < p1TargetRows) {
      const availItems = Math.max(1, p1TargetRows - p1RowsCount - 1)
      const p1Items = cat.items.slice(0, availItems)
      const p2Items = cat.items.slice(availItems)

      page1Categories.push({
        key: cat.key,
        label: cat.label,
        items: p1Items,
        isContinued: false
      })
      p1RowsCount += 1 + p1Items.length

      if (p2Items.length > 0) {
        page2Categories.push({
          key: cat.key,
          label: cat.label,
          items: p2Items,
          isContinued: true
      ...
      page2Categories.push({ ...cat, isContinued: false })
    }
  }
  ...
  return [
    {
      categories: page1Categories,
      showTop: true,
      showSignatureBlock: false,
    },
    {
      categories: page2Categories,
      showTop: false,
      showSignatureBlock: true,
    }
  ]
    ...
    >
      ...
      <div className="mb-3 print:hidden flex items-center gap-2.5 bg-white/95 dark:bg-[#1A1A1A]/95 backdrop-blur-md px-3.5 py-1 rounded-full border border-border shadow-xs z-10 select-none">
        ...
        <span className="text-[11px] font-bold text-emerald-600 dark:text-emerald-400 font-mono">
          📋 Material Dispatch & Packing List ({totalPages} {totalPages === 1 ? 'Page' : 'Pages'} A4)
        </span>
      </div>
      ...
      {virtualPages.map((page, pageIdx) => (
        <div 
          key={pageIdx} 
          className={cn(
            "w-full flex justify-center mb-6 print:block print:m-0 print:p-0",
            pageIdx < totalPages - 1 && "print-break"
          )}
        >
          <div 
            ...
            >
              ...
              <div className="space-y-3">
                ...
                {page.showTop ? (
                  ...
                  /* Compact Header (Page 2 Only) */
                  <div className="flex justify-between items-center border-b border-[#111111]/20 pb-2 mb-2">
                    <div className="flex items-center gap-2">
                      <p className="font-extrabold text-sm text-[#111111] uppercase tracking-tight">
                        {invoice.fromName || 'MG SOLAR'}
                      </p>
                      <span className="text-[9.5px] text-[#777777]">
                        • DOC #: MG-CL-{invoice.invoiceNumber || '260715133721'}
                      </span>
                    </div>
                    <div className="text-right flex items-center gap-3">
                      <span className="text-[9.5px] text-[#666666] font-sans">
                        CLIENT: <strong className="text-[#111111] font-mono">{invoice.toName || '—'}</strong> | DATE: <strong className="text-[#111111] font-mono">{formatDate(invoice.issueDate)}</strong> | STATUS: <strong className="text-[#008B4C] font-mono">{checkedCount}/{totalCount} Verified</strong>
                      </span>
                      <span className="text-[9px] font-bold bg-[#111111] text-white px-2 py-0.5 rounded-xs uppercase tracking-wider">
                      ...
                      <Fragment key={cat.key}>
                        <tr className="bg-[#111111] text-white">
                          <td colSpan={5} className="py-1 px-2 text-[8.5px] font-bold uppercase tracking-wider">
                            ▸ {cat.label} ({cat.items.length}){cat.isContinued ? ' (CONT.)' : ''}
                          </td>
                        </tr>
                        {cat.items.map(item => {
              ...
              {page.showSignatureBlock ? (
                ...
                <div className="pt-2 border-t border-[#111111]/20 flex justify-between items-center text-[8px] text-[#888888] font-sans">
                  <span>MG SOLAR Material Dispatch Verification Form — Official Document</span>
                  <span>Page {pageIdx + 1} of {totalPages}</span>
                </div>
              )}
