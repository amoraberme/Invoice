...
**Key files:**
- `app/page.tsx` — sidebar form UI, orchestrates state via `useMGInvoice`
...
- `lib/use-mg-invoice.ts` — `useMGInvoice()` hook: invoice state, auto-save, update callbacks
- `lib/utils.ts` — `cn()`, `formatDate()`, `formatCurrency()`
- `components/mg-invoice-preview.tsx` — A4 preview component, manages its own scale/ResizeObserver
...
`components/mg-invoice-preview.tsx` renders the invoice at true A4 CSS dimensions (`794×1123px`, defined in `lib/constants.ts`) and uses `transform: scale()` driven by a `ResizeObserver` to fit the canvas — content never reflows, only scales. Print resets the transform so the browser prints at exact A4 size.
...
`lib/store.ts` opens `mg-invoice-db` (version 1), object store `data`, single key `current-invoice`. Auto-save is debounced 400ms after any state change, managed inside `useMGInvoice()`. Silently fails if unavailable.

## URL Params API

`useMGInvoice()` reads URL params on load and applies them over saved state. Any invoice field name is a valid param (e.g. `?fromName=Acme&vatRate=20`). The URL stays in sync as the user types via `history.replaceState`. Non-invoice params (e.g. `print=true`) are preserved. `?print=true` auto-triggers `window.print()` after load.
