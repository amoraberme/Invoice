...
function calculateSystemCost(systemKw, isHybrid) {
  ...
  // On-grid inverter discount? In the prompt, the user says "on grid and hybrid options". Let's assume on-grid inverter price is the same or maybe we can make a custom pricing if we want. But let's assume same inverter base price for simplicity.
  total += inverterPrice;
...
console.log("Size\tHybrid\tOn-Grid");
sizes.forEach(s => {
  const hybrid = calculateSystemCost(s, true);
  const ongrid = calculateSystemCost(s, false);
  console.log(`${s}kW\t₱${hybrid.toLocaleString(undefined, {minimumFractionDigits: 2})}\t₱${ongrid.toLocaleString(undefined, {minimumFractionDigits: 2})}`);
});
